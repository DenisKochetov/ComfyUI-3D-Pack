...
class Hunyuan3DPaintPipeline:
    ...
    def from_pretrained(cls, model_path):
        ...
        if os.path.exists(model_path):
            ...
            # Ищем paint модель (сначала турбо, потом обычную)
            multiview_model_path = None
            for paint_variant in ['hunyuan3d-paint-v2-0-turbo', 'hunyuan3d-paint-v2-0']:
                candidate_path = os.path.join(model_path, paint_variant)
                if os.path.exists(candidate_path):
                    multiview_model_path = candidate_path
                    print(f"Найдена paint модель: {paint_variant}")
                    break
            
            if os.path.exists(delight_model_path) and multiview_model_path:
                print(f"Используем локальные модели:")
                print(f"  Delight: {delight_model_path}")
                print(f"  Paint: {multiview_model_path}")
