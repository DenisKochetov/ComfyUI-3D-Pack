# Hunyuan3D-2.1 без зависимости от trimesh для GLB

class Hunyuan3D_21_TexGen_NoTrimesh:
    """Hunyuan3D-2.1 Texture Generation БЕЗ зависимости от trimesh для GLB"""
    ...
    def generate(self, texgen_pipe, mesh_path, image, create_pbr, use_remesh):
        ...
        try:
            ...
            if create_pbr:
                ...
                # Загружаем GLB НАШИМ загрузчиком (БЕЗ trimesh)
                if os.path.exists(glb_path):
                    try:
                        print(f"🚀 Загружаем GLB через mesh_processor: {glb_path}")
                        mesh_out = Mesh.load_gltf(glb_path)
                        
                        if mesh_out is not None:
                            mesh_out.auto_normal()
                            print(f"✅ GLB загружен успешно через mesh_processor!")
                            ...
                            print("❌ mesh_processor вернул None")
                            
                    except Exception as e:
                        print(f"❌ Ошибка загрузки GLB через mesh_processor: {e}")
                        mesh_out = None
            
            # Если GLB не удался, загружаем OBJ
            if mesh_out is None:
                try:
                    print(f"🔄 Fallback: загружаем OBJ через mesh_processor: {result_path}")
                    ...
                    if mesh_out is not None:
                        ...
                        raise Exception("mesh_processor вернул None для OBJ")
                        
                except Exception as e:
                    print(f"❌ Критическая ошибка загрузки OBJ: {e}")
...
class Hunyuan3D_21_ShapeGen_NoTrimesh:
    """Hunyuan3D-2.1 Shape Generation с использованием TrimeshLike вместо trimesh"""
    ...
    def generate(self, shapegen_pipe, image, seed, steps, guidance_scale, octree_resolution, remove_background, auto_cleanup):
        ...
        # Здесь мы всё еще используем trimesh объект от пайплайна, 
        # но конвертируем его в наш формат
        try:
            print("🔄 Конвертируем trimesh объект в Mesh...")
            mesh_out = Mesh.from_trimesh(trimesh_obj, use_new_converter=True)
            
            if mesh_out is not None:
                mesh_out.auto_normal()
                print("✅ Конвертация через новый конвертер успешна")
            else:
            ...
            print("🆘 Fallback к старому методу...")
            mesh_out = Mesh.load_trimesh(given_mesh=trimesh_obj)
            mesh_out.auto_normal()
...
# Вспомогательная функция для полной замены trimesh.load
def load_mesh_no_trimesh(path):
    """Загружает меш БЕЗ использования trimesh для GLB/GLTF"""
    
    if path.lower().endswith(('.glb', '.gltf')):
        print(f"🚀 Загружаем {path} через mesh_processor...")
        ...
        print(f"🚀 Загружаем {path} через mesh_processor OBJ loader...")
        mesh = Mesh.load_obj(path)
        return TrimeshLike(mesh)
        
    else:
        # Для других форматов пока используем оригинальный trimesh
        print(f"⚠️ Формат {path} не поддерживается mesh_processor, используем trimesh...")
        import trimesh
